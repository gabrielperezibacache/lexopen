...
export async function POST(req: NextRequest) {
  try {
    ...
    const days = Number.isFinite(daysParam) ? Math.max(0, Math.min(30, Math.trunc(daysParam))) : 3;
    ...
    return NextResponse.json({ ok: true, days, plazos: plazos.length, notifications });
  } catch (e) {
