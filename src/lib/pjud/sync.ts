...
export async function syncCausaPjud(
  ...
  await prisma.$transaction(async (tx) => {
    for (const m of fetchResult.movimientos) {
      const existing = await tx.causaMovimiento.findFirst({
        where: {
          causaId,
          OR: [
      ...
      await tx.causaMovimiento.create({
        data: {
          causaId,
          titulo: m.titulo,
      ...
      inserted += 1;
      if (m.relevante) newRelevant.push(m.titulo);
    }
    ...
    if (newRelevant.length && causa.abogadoId) {
